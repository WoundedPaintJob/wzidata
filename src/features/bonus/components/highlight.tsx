...
const BonusHighlight = (props: { bonus: BonusState }) => {
  ...
  const remainingBonusZones = zones.filter((z) => !z.Conquered && props.bonus.ZoneIds.includes(z.Id));
  ...
  remainingBonusZones.forEach((zone) => {
    cost += totalCostForZone(zone, conqueredHospitals, hospitalSaveMultiplier, jointStrikeMultiplier, zone.ConnectedZones.length > 1);
  })
