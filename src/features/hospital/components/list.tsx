...
const HospitalList = () => {
  ...
  function conquerFree(): void {
    ...
    zones.forEach((z) => {
      if (!z.Conquered) {
        if (z.ConnectedZones.filter((cz) => zones.get(cz).Conquered).length > 0) {
          let hospitalSaves = 0;
          hospitals.forEach(
            (hospital) => hospitalSaves += isAssetConquered(hospital, zones, bonuses) ?
              hospitalSaveForZone(hospital, z) * hospitalMultiplier : 0
          );
          if (hospitalSaves > z.Cost)
            freeZones.push(z);
        }
      }
    })
    if (freeZones.length > 0)
      conquerZones(freeZones);
  }
