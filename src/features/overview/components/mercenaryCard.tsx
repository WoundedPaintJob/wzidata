...
const MercenaryCard = (props: { data: CardData }) => {
  ...
  const remainingMercZones = props.data.unconqueredZones.filter(
    (z) => z.Reward.Type == RewardType.MercenaryCamp
  );
  const remainingMercBonuses = props.data.unconqueredBonuses.filter(
